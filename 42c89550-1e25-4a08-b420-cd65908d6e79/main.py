...
class TradingStrategy(Strategy):
    ...
    The final "ROAR Score" determines the allocation between SPY (risk-on) and BIL (risk-off).
    A higher score increases exposure to SPY, while a lower score shifts capital to BIL.
    Rebalancing occurs weekly.

    Updated Allocation Formula (Oct 2025):
        SPY = 50% (fixed) + (ROAR Score × 50%)
        BIL = 1 - SPY
        → Example: ROAR = 20 → 60% SPY, 40% BIL
    """
    def __init__(self):
    ...
    def strength_by_barchart_method(self, close_series, period):
        ...
        if period == 20:
            ...
            thresholds = [-0.05, 0.04, 0.08, 0.12]
        else:
            thresholds = [-0.05, 0.05, 0.10, 0.15]
        if pd.isna(pct) or (pct > thresholds[1] and pct <= thresholds[2]):
            return "Average"
        if pct <= thresholds[0]:
            return "Weak"
        if pct <= thresholds[1]:
    ...
    def realized_vol_score(self, close, lookback=126, window=21):
        ...
        rank = sum(val > dec for dec in deciles)
        score = 10 - (rank * (20 / 9))
        return score
